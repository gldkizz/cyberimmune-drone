...
При запуске `make online` полётное задание подтверждается через ОРвД, а значит в коде нужно учитывать, получил ли полётный контроллер задание. В ином случае логика может отработать до его получения, и ничего не будет выведено на экран.
...
## Вопрос 13: При исполнении `./run.sh` ошибка об отсутствии директории MAVProxy. Что делать?
